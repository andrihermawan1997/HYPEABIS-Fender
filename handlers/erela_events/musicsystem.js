...
function generateQueueEmbed(client, guildId, leave) {
  ...
  var embeds = [
    new MessageEmbed()
    .setColor("#A3C7FD")
    .setImage("https://cdn.discordapp.com/attachments/922681442885054565/923389651253882900/HYPEABIS_FENDER_BANNER.png"),
    new MessageEmbed()
    .setColor("#A3C7FD")
    // .setFooter(es.footertext, guild.iconURL({
    //   dynamic: true
    // }))
    .setImage("https://cdn.discordapp.com/attachments/922681442885054565/923389650926698586/HYPEABIS_FENDER_BACKGROUND.png")
  ...
  if (!leave && player && player.queue && player.queue.current) {
    embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
      .setFooter(`Song By: ${player.queue.current.author} | Queue length: ${player.queue.length} Songs | Duration: ${format(player.queue.current.duration).split(" | ")[0]}`)
      //.addField(`Duration: `, `\`${format(player.queue.current.duration).split(" | ")[0]}\` | \`${format(player.queue.current.duration).split(" | ")[1]}\``, true)
      // .addField(`Song By: `, `\`${player.queue.current.author}\``, true)
      // .addField(`Queue length: `, `\`${player.queue.length} Songs\``, true)
      .setAuthor(`${player.queue.current.title}`, "https://emoji.gg/assets/emoji/3662-frank-music.gif", player.queue.current.uri)
